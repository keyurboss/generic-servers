...
func Emails() {
	...
	for i, row := range RowsData {
		...
		if attacheMentName != "" {
			attacheMentNames := strings.Split(attacheMentName, "@@")
			for _, attattacheMentNameFile := range attacheMentNames {
				attacheMentNamePath := filepath.Join(ThisConfig.BasePathForAttachment, attattacheMentNameFile)
